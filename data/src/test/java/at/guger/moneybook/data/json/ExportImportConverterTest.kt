...
/**
 * Test class for [ExportImportConverter].
 */
class ExportImportConverterTest {

    private lateinit var exportImportConverter: ExportImportConverter

    @Before
    fun setUp() {
        exportImportConverter = ExportImportConverter()
    }

    @Test
    fun testExportImport() = runBlocking {
        val export = exportImportConverter.convertExport(EXPORT_MODEL)

        val import = exportImportConverter.convertImport(export)

        assertTransaction(Transaction(import.transactionEntities.first(), import.accounts.first(), import.budgets.first(), import.contacts))
    }
    ...
    companion object {
        ...
        val EXPORT_MODEL = ExportImportConverter.ExportImportModel(
            listOf(TRANSACTION.entity),
            listOf(TRANSACTION.account!!),
            TRANSACTION.contacts!!,
            listOf(),
            listOf(TRANSACTION.budget!!)
        )
    }
}