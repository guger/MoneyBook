...
internal interface AccountsDao {
    ...
    @Query(
        """
            ...
            ELSE -transactions.value END) FROM transactions WHERE transactions.account_id = accounts.id AND strftime('%Y-%m-%d', transactions.date / 1000, 'unixepoch', 'localtime') <= date('now', 'localtime')) AS balance
            FROM accounts
